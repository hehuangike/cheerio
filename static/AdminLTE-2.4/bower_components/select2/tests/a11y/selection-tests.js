...
test('aria-expanded reflects the state of the content', function (assert) {
  var $select = $('#qunit-fixture .single');
  ...
  assert.equal(
    ...
    'false',
    'The content should not be expanded when it is closed'
  );
  ...
  assert.equal(
    $selection.attr('aria-expanded'),
    'true',
    'The content should be expanded when it is opened'
  );
});
...
test('static aria attributes are present', function (assert) {
  ...
  assert.equal(
    $selection.attr('role'),
    'combobox',
    'The content should identify as a combobox'
  );

  assert.equal(
    $selection.attr('aria-haspopup'),
    'true',
    'The dropdown is considered a popup of the content'
  );
});

test('the content should be in the tab order', function (assert) {
  var $select = $('#qunit-fixture .single');
...
test('aria-labelledby should match the rendered content', function (assert) {
  var $select = $('#qunit-fixture .single');
  ...
  assert.equal(
    ...
    $rendered.attr('id'),
    'The rendered selection should label the content'
  );
});
